...
export class AppService {
  ...
  constructor(
    ...
    private readonly uiNotificationModel: ReturnModelType<
      typeof UiNotification
    >,
  ) {}

  async emailNotificationChannel(
    notification: CreateNotificationRequest,
    details: NotificationMapperType,
  ): Promise<boolean> {
    ...
    this.logger.log(
      `Sending email to <${user.firstName} ${user.lastName}@${user.email}>...`,
    );
  ...
  async uiNotificationChannel(
    ...
    this.logger.log(
      `UI Notification for UserID ${notification.userId} was created.`,
    );
  ...
  async getUser(userId: number): Promise<User | null> {
    const user = await axios.get<User>(`${apiUrl}/user/${userId}`);
    return user.data;
  }

  async getCompany(companyId: number): Promise<Company | null> {
    const company = await axios.get<Company>(`${apiUrl}/company/${companyId}`);
    return company.data;
  }

  async getNotificationChannelByUserId(
    userId: number,
  ...
  async getNotificationChannelByCompanyId(
    companyId: number,
  ): Promise<NotificationChannel[] | null> {
  ...
  async retrieveUiNotificationsForUser(
    userId: number,
    ...
    this.changeUiNotificationStatus(
